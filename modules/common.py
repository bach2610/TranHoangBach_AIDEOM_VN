# -*- coding: utf-8 -*-
"""
modules/common.py — Thành phần dùng chung cho toàn hệ thống AIDEOM-VN.

Chứa: hàm nạp dữ liệu (3 tệp CSV gốc), bảng tên tiếng Việt, các hằng số mô hình
Cobb-Douglas, hàm sản xuất lõi, bảng màu, nội dung Câu hỏi thảo luận chính sách
(Bài 1->12) và các tiện ích hiển thị (header, policy_box).
"""
import numpy as np
import pandas as pd
...
def prepared_data():
    """Nạp 3 tệp CSV và gắn thêm cột tên tiếng Việt; trả về (macro, sectors, regions)."""
    macro = load_macro()
...
POLICY_QA = {
    ...
    11: [
        ...
        ("c", "Mục 11 của bài báo nguồn nhấn mạnh “AI không thay thế quyết định chính trị - xã hội”. Em sẽ tích hợp $\\pi^*$ vào quy trình hoạch định chính sách Việt Nam như thế nào để không vi phạm nguyên tắc này?", "Để tích hợp mô hình học tăng cường $\\pi^*$ vào quy trình hoạch định chính sách thực tế tại Việt Nam mà không vi phạm nguyên tắc tối thượng trên, em đề xuất mô hình \"Human-in-the-loop\" (Con người kiểm soát trung tâm) theo cơ chế 3 bước:\n\n_Giai đoạn Khuyến nghị_ (AI làm Cố vấn): Mô hình $\\pi^*$ sẽ đóng vai trò là một \"trợ lý ảo vĩ mô\" cho Bộ Kế hoạch và Đầu tư hoặc Bộ Thông tin và Truyền thông. Dựa trên dữ liệu thời gian thực, AI sẽ chạy giả lập và đưa ra bảng thực đơn các phương án đánh đổi (Trade-off Menu) kèm theo dự báo xác suất thành công.\n\n_Giai đoạn Thẩm định_ (Hội đồng chuyên gia): Các khuyến nghị của AI (hộp đen) buộc phải đi qua Hội đồng thẩm định liên Bộ và các nhà khoa học để phản biện, bóc tách thuật toán, đánh giá các tác động định tính về mặt chính trị, lòng dân, và an sinh xã hội – những thứ mà AI không thể số hóa được.\n\n_Giai đoạn Phê duyệt_ (Quyết định chính trị): Quyết định bấm nút tối ưu cuối cùng hoàn toàn thuộc về ý chí chính trị của Chính phủ và Quốc hội. Chính phủ có quyền bác bỏ phương án có điểm số GDP cao nhất của AI nếu phương án đó vi phạm các giá trị cốt lõi về chủ quyền quốc gia hoặc làm gia tăng bất bình đẳng xã hội. AI chỉ định hình không gian giải pháp, còn Con người định hình giá trị quốc gia."),
    ],
    12: [
